...
(async () => {
  ...
  await page.getByText('Ich prüfe Ihre Frage und die verfügbaren Daten …',{exact:true}).waitFor();
  assert.equal(await page.locator('#aiQuestionInput').inputValue(),'');
  assert.equal(await page.locator('.ki-working-spinner').count(),1);
  assert.equal(await page.locator('.ki-message-working').getAttribute('aria-busy'),'true');
  ...
  assert.equal(await page.locator('.ki-working-spinner').count(),0);
  ...
  assert.equal(await page.locator('#aiQuestionInput').inputValue(),'');
  assert.equal(await page.locator('#aiWorking').count(),0);
