...
Die NST-2007-Schlüssel werden bereits beim Einlesen als Zeichenfolgen behandelt. Eine führende Null ist Bestandteil des fachlichen Schlüssels und darf weder durch automatische Typumwandlung noch durch eine Tabellenkalkulation entfernt werden. Schiene und Binnenschiff liefern dreistellige Feinpositionen wie `011`, `01A` oder `192`; die ersten beiden Stellen bezeichnen die NST-2007-Abteilung. Die Zuordnung zu den sieben Hauptgruppen muss in allen betroffenen Pipelines aus derselben kanonischen Zuordnung stammen. Vor einem Release muss `python scripts/validate_nst_fine_codes.py` bestehen. Der Test verwirft unbekannte Codeformen und rechnet die Summen jeder C1–C7-Gruppe aus den Feinpositionen gegen `fact_od_flows.parquet` nach.
...
| `scripts/validate_nst_fine_codes.py` | keine Produktivdatei | prüft die NST-2007-Feinpositionen von Schiene und Binnenschiff, ihre C1–C7-Summen und die sichtbaren NST-20-/C1–C7-Bezeichnungen |
