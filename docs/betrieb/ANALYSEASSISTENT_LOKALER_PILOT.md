...
Tabellen und feste Kurzfassungen werden aus den Abfragewerten erzeugt. Ein optionaler zweiter Modellaufruf darf ausschließlich vorhandene Aussagen auswählen. Ungültige Kennungen, freie Zusatzfelder oder ein Modellfehler ersetzen keine Fakten; Pflichtangaben und Tabellen bleiben erhalten. Die Regeln sind im Python-Code implementiert. `FACHREGELN.json` bleibt die Version 0.1.0 der Spezifikation mit deaktivierter allgemeiner Produktfreigabe. Der lokale Pilot ist durch den ausdrücklichen Programmaufruf aktiviert; er aktiviert keine Portalrechte und erhebt keinen Anspruch auf Umsetzung aller deklarativen Regeln.
...
Die Implementierung verwendet `https://router.eu.requesty.ai/v1`, folgt dem dokumentierten [Requesty-EU-Routing](https://docs.requesty.ai/features/eu-routing) und den [strukturierten Ausgaben](https://docs.requesty.ai/features/structured-outputs). Planantworten werden als JSON-Objekt angefordert und lokal streng geprüft, weil ihre Parameter je Funktion variieren. Die optionale Aussageauswahl verwendet ein festes JSON-Schema. Die tatsächliche Kompatibilität und Laufzeit beider Phasen mit dem eingetragenen Modell ist noch nicht durch einen Fachfragenlauf belegt. Die in Requesty eingerichteten Datenschutz-/Aufbewahrungseinstellungen wurden nicht unabhängig geprüft.
