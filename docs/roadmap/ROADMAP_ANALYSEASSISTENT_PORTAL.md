...
### Beleggebundene Antwortsynthese und Mehrjahresrelation – lokal (11.09.2026)

Die lokale Version 0.2.0 setzt die Ergebnisse nicht mehr nur in Tabellen um. Für die numerischen Fragetypen berechnet der Server Kernaussagen, die Werte miteinander in Beziehung setzen. Ein optionaler Antwortaufruf formuliert daraus höchstens drei Absätze und verweist je Absatz auf die verwendeten Belege. Der Server lehnt fremde Kennungen, neue Zahlen, veränderte Einheiten sowie unbelegte Ursachen, Bewertungen und Empfehlungen ab. Die analytische Serverfassung bleibt als Rückfall erhalten; lange Tabellen werden zunächst eingeklappt.

Der konkrete Rosenheim–Augsburg-Fall wird als `relation_history` für 2020–2024 erkannt, einschließlich der kurzen Folgeantwort „mehrere Jahrgänge“. Drei `gpt-5.6-terra`-Subagenten prüften ausgewählte Fälle, Promptgrenzen und technische Robustheit. 77/77 lokale Prüfungen bestehen; ein zusätzlicher strukturierter Katalogstichprobenlauf deckt zwölf numerische beziehungsweise methodische Fälle ab. Nachweis: `outputs/analyseassistent_runtime_20260911/narrative_validation01.json`.

Nächster Schritt ist ein neuer Testrelease mit kleinem echtem Modell-Stichprobenlauf und angemeldeter Browserprüfung. Bis dahin bleibt ui10 aktiv; keine Produktionsänderung und keine neue 45-Fälle-Gesamtabnahme.
